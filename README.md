...
두 가지 방법 중 아무거나 쓰면 됩니다. 둘 다 처리 중이던 행은 끝까지 마친 뒤
(중간에 끊지 않음) 다음 행 시작 전에 멈춥니다.

1. **F8 키 누르기** — 어느 창에 포커스가 있든 상관없이 눌리면 인식됩니다.
   본체가 자동화 동작으로 바쁜 동안에도 놓치지 않도록, 키보드 상태를 별도
   스레드에서 40ms마다 확인합니다(`GetAsyncKeyState` 사용 — 창 포커스나
   Windows 메시지 처리에 의존하지 않는 방식이라, 자동화 중에도 확실히 잡힘).
   원격 접속(VDI) 클라이언트가 F8을 가로채서 안 먹히는 경우에는 다른 키로
   바꿔서 실행할 수 있습니다:

   ```powershell
   # F9로 바꿔서 실행 (F7=0x76 F8=0x77 F9=0x78 F10=0x79 F11=0x7A F12=0x7B)
   .\Update-EmsAlarms.ps1 -StopKeyCode 0x78 -StopKeyName "F9"
   ```

2. **`STOP.txt` 파일 만들기** — 스크립트와 같은 폴더에 그 이름의 빈 텍스트
   파일을 만들어두면 멈춥니다. 키가 어떤 이유로든 안 먹힐 때의 예비 수단입니다.
